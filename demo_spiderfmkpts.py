import json
from spider.model import SPIDER, SPIDER_MAST3R, SPIDER_FMwarp, SPIDER_FM
import torch
import numpy as np
import random
...
def spiderfm_match_path(save_dir, im_A_path, im_B_path, model, device = 'cuda', coarse_size=512, fine_size=None, name='spider'):
    ...
    def draw_matches(save_dir, kpts1, kpts2, mconf, name='mast3r_matches.png', n_viz = 400):
        ...
        #     pl.scatter(x0, y0, color=cmap(i / (n_viz - 1)), s=0.5)
        #     pl.scatter(x1 + W0, y1, color=cmap(i / (n_viz - 1)), s=0.5)
        # # pl.show(block=True)
    ...
    return draw_matches(save_dir, kpts1, kpts2, mconf, name=name)
...
if __name__ == '__main__':
    ...
    #         with torch.no_grad():
    #             spiderfm_match_path(save_path, im_A_path, im_B_path, model, device, coarse_size=512, fine_size=512, name='spiderfm512')
    tot_0 = []
    tot_1 = []
    print(len(pairs))
    for idx, pair in enumerate(tqdm(pairs)):
        ...
        try:
            with torch.no_grad():
                num_matches = spiderfm_match_path(save_path, im_A_path, im_B_path, model, device, coarse_size=512, fine_size=512, name='spiderfm512')
            if pair[2] == 0:
