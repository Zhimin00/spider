import json
from spider.benchmarks_new import AerialMegaDepthPoseEstimationBenchmark, MegaDepthPoseEstimationBenchmark
...
def test_aerial_concat(model1, model2, name, coarse_size=512, fine_size=1600):
    aerial_benchmark = AerialMegaDepthPoseEstimationBenchmark('/cis/net/io99a/data/zshao/megadepth_aerial_data/megadepth_aerial_processed')
    aerial_results = aerial_benchmark.benchmark_concat(model1, model2, coarse_size=coarse_size, fine_size=fine_size)
    json.dump(aerial_results, open(f"./results/aerial_{name}.json", "w"))

def test_mega1500_concat(model1, model2, name, coarse_size=512, fine_size=1600):
    mega1500_benchmark = MegaDepthPoseEstimationBenchmark('/cis/net/r24a/data/zshao/data/megadepth/megadepth_test_1500')
    mega1500_results = mega1500_benchmark.benchmark_concat(model1, model2, coarse_size=coarse_size, fine_size=fine_size)
    json.dump(mega1500_results, open(f"./results/mega1500_{name}.json", "w"))

if __name__ == "__main__":
    device = "cuda"
    parser = ArgumentParser()
    parser.add_argument("--exp_name", default='concat', type=str)
    ...
    mast3r_model = AsymmetricMASt3R.from_pretrained("./model_weights/aerial-mast3r.pth").to(device)
    ...
    if args.dataset == 'mega1500_concat':
        test_mega1500_concat(model, mast3r_model, experiment_name, coarse_size=args.coarse_size, fine_size=args.fine_size)
    elif args.dataset == 'aerial':
        test_aerial_concat(model, mast3r_model, experiment_name, coarse_size=args.coarse_size, fine_size=args.fine_size)