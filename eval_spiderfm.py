...
def test_mega(model, device, name, coarse_size, fine_size=None):
    ...
    print(f'{name} megadepth auc:{auc}')
...
def test_scannet(model, device, name, coarse_size, fine_size=None):
    ...
    print(f'{name} scannet auc: {auc}')
    json.dump(results, open(f"./mast3r_results/scannet_{name}.json", "w"))

if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument("--exp_name", default='spiderfm_nogate', type=str)
    ...
    # model = SPIDER_FM.from_pretrained('/cis/net/r24a/data/zshao/checkpoints/fm/spiderfm_mlp_ms_0813_1e-4/checkpoint-best.pth').to(device)
    model = SPIDER_FM.from_pretrained('/cis/home/zshao14/checkpoints/spider_msfr_nogate/checkpoint-best.pth').to(device)
