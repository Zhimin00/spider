...
# def select_scene(scene, *instances, opposite=False):
#     with np.load(osp.join('/cis/home/cpeng/dust3r/data/megadepth_dataset_processed', 'all_metadata.npz')) as data:
#         all_scenes = data['scenes']
#         all_images = data['images']
#         pairs = data['pairs']
#     scenes = (scene,) if isinstance(scene, str) else tuple(scene)
#     scene_id = [s.startswith(scenes) for s in all_scenes]
#     assert any(scene_id), 'no scene found'

#     valid = np.in1d(pairs['scene_id'], np.nonzero(scene_id)[0])
#     if instances:
#         image_id = [i.startswith(instances) for i in all_images]
#         image_id = np.nonzero(image_id)[0]
#         assert len(image_id), 'no instance found'
#         # both together?
#         if len(instances) == 2:
#             valid &= np.in1d(pairs['im1_id'], image_id) & np.in1d(pairs['im2_id'], image_id)
#         else:
#             valid &= np.in1d(pairs['im1_id'], image_id) | np.in1d(pairs['im2_id'], image_id)
...
# pairs = select_scene(('0015', '0022'))
        ...
        #     e_t, e_R = compute_pose_error_T(T1_est, T2_est, R, t)
        #     e_pose = max(e_t, e_R)        
        #     tot_e_t.append(e_t)
        #     tot_e_R.append(e_R)
...
if __name__ == '__main__':
    ...
    output_file_path = '/cis/net/r24a/data/zshao/data/mega1500_result'
