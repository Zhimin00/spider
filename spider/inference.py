...
from mast3r.inference import coarse_to_fine_new
...
def spider_mast3r_symmetric_inference(model, img1, img2, device):
    ...
    def decoder(feat1, feat2, pos1, pos2, shape1, shape2, cnn_feats1, cnn_feats2):
        ...
        with warnings.catch_warnings():
            ...
            with torch.cuda.amp.autocast(enabled=False):
                corresps = model._downstream_head(1, cnn_feats1 + [feat16_1], cnn_feats2 + [feat16_2], shape1, shape2)
                res1 = model._downstream_fmhead(2, cnn_feats1, shape1, upsample = False, low_desc = None, low_certainty = None)
                res2 = model._downstream_fmhead(3, cnn_feats2, shape2, upsample = False, low_desc = None, low_certainty = None)
...
@torch.no_grad()
def spider_mast3r_symmetric_inference_upsample(model, img1_coarse, img2_coarse, img1, img2, device):
    # combine all ref images into object-centric representation
    low_corresps, res = spider_mast3r_symmetric_inference(model, img1_coarse, img2_coarse, device)
