...
from spider.utils.misc import interleave_list, transpose_to_landscape_warp, transpose_to_landscape_fm, transpose_to_landscape_fmwarp, transpose_to_landscape_fm_PasF
...
class SPIDER_two (CroCoNet):
    ...
    def __init__(self,
        ...
        self.cnn1 = VGG19_all(pretrained=vgg_pretrained)
        self.cnn2 = VGG19_all(pretrained=vgg_pretrained)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kw):
        if os.path.isfile(pretrained_model_name_or_path):
            ...
            try:
                model = super(SPIDER_two, cls).from_pretrained(pretrained_model_name_or_path, **kw)
    ...
    def cnn_embed1(self, img, true_shape):
        B, C, H, W = img.shape
        if self.landscape_only:
            ...
            cnn_feats = self.cnn1(img)
        return cnn_feats
    
    def cnn_embed2(self, img, true_shape):
        B, C, H, W = img.shape
        if self.landscape_only:
            assert W >= H, f'img should be in landscape mode, but got {W=} {H=}'
            assert true_shape.shape == (B, 2), f"true_shape has the wrong shape={true_shape.shape}"
            ...
            feat1[is_landscape], feat2[is_landscape], feat4[is_landscape], feat8[is_landscape] = self.cnn2(img[is_landscape])
            feat1[is_portrait], feat2[is_portrait], feat4[is_portrait], feat8[is_portrait] = self.cnn2(img[is_portrait].swapaxes(-1, -2))
            cnn_feats = [feat1, feat2, feat4, feat8]
        else:
            cnn_feats = self.cnn2(img)
        return cnn_feats

    def _encode_image(self, image, true_shape):
        # embed the image into patches  (x has size B x Npatches x C)
        x, pos = self.patch_embed(image, true_shape=true_shape)
        cnn_feats_warp = self.cnn_embed1(image, true_shape=true_shape)
        cnn_feats_fm = self.cnn_embed2(image, true_shape=true_shape)
        ...
        x = self.enc_norm(x)
        return x, pos, cnn_feats_warp, cnn_feats_fm

    def _encode_image_pairs(self, img1, img2, true_shape1, true_shape2):
        if img1.shape[-2:] == img2.shape[-2:]:
            out, pos, cnn_feats_warp, cnn_feats_fm = self._encode_image(torch.cat((img1, img2), dim=0),
                                             torch.cat((true_shape1, true_shape2), dim=0))
            out, out2 = out.chunk(2, dim=0)
            pos, pos2 = pos.chunk(2, dim=0)
            cnn_feats_warp, cnn_feats_warp2 = zip(*[feat.chunk(2, dim=0) for feat in cnn_feats_warp])
            cnn_feats_warp = list(cnn_feats_warp)
            cnn_feats_warp2 = list(cnn_feats_warp2)

            cnn_feats_fm, cnn_feats_fm2 = zip(*[feat.chunk(2, dim=0) for feat in cnn_feats_fm])
            cnn_feats_fm = list(cnn_feats_fm)
            cnn_feats_fm2 = list(cnn_feats_fm2)
        else:
            out, pos, cnn_feats_warp, cnn_feats_fm = self._encode_image(img1, true_shape1)
            out2, pos2, cnn_feats_warp2, cnn_feats_fm2 = self._encode_image(img2, true_shape2)
        return out, out2, pos, pos2, cnn_feats_warp, cnn_feats_warp2, cnn_feats_fm, cnn_feats_fm2
    ...
    def _encode_symmetrized(self, view1, view2):
        ...
        if is_symmetrized(view1, view2):
            ...
            feat1, feat2, pos1, pos2, cnn_feats_warp1, cnn_feats_warp2, cnn_feats_fm1, cnn_feats_fm2 = self._encode_image_pairs(img1[::2], img2[::2], shape1[::2], shape2[::2])
            feat1, feat2 = interleave(feat1, feat2)
            pos1, pos2 = interleave(pos1, pos2)
            cnn_feats_warp1, cnn_feats_warp2 = interleave_list(cnn_feats_warp1, cnn_feats_warp2)
            cnn_feats_fm1, cnn_feats_fm2 = interleave_list(cnn_feats_fm1, cnn_feats_fm2)
        else:
            feat1, feat2, pos1, pos2, cnn_feats_warp1, cnn_feats_warp2, cnn_feats_fm1, cnn_feats_fm2  = self._encode_image_pairs(img1, img2, shape1, shape2)

        return (shape1, shape2), (feat1, feat2), (pos1, pos2), (cnn_feats_warp1, cnn_feats_warp2), (cnn_feats_fm1, cnn_feats_fm2)

    def _encode_image_upsample(self, image, true_shape):
        cnn_feats = self.cnn_embed1(image, true_shape=true_shape)
        return cnn_feats
    ...
    def _encode_image_onlyfm(self, image, true_shape):
        # embed the image into patches  (x has size B x Npatches x C)
        x, pos = self.patch_embed(image, true_shape=true_shape)
        cnn_feats_fm = self.cnn_embed2(image, true_shape=true_shape)
        # add positional embedding without cls token
        ...
        x = self.enc_norm(x)
        return x, pos, cnn_feats_fm

    def _encode_image_pairs_onlyfm(self, img1, img2, true_shape1, true_shape2):
        if img1.shape[-2:] == img2.shape[-2:]:
            out, pos, cnn_feats_fm = self._encode_image_onlyfm(torch.cat((img1, img2), dim=0),
                                             torch.cat((true_shape1, true_shape2), dim=0))
            out, out2 = out.chunk(2, dim=0)
            pos, pos2 = pos.chunk(2, dim=0)
            ...
            out, pos, cnn_feats_fm = self._encode_image_onlyfm(img1, true_shape1)
            out2, pos2, cnn_feats_fm2 = self._encode_image_onlyfm(img2, true_shape2)
        return out, out2, pos, pos2, cnn_feats_fm, cnn_feats_fm2

    def _encode_symmetrized_onlyfm(self, view1, view2):
        img1 = view1['img']
        img2 = view2['img']
        B = img1.shape[0]
        # Recover true_shape when available, otherwise assume that the img shape is the true one
        ...
        if is_symmetrized(view1, view2):
            # computing half of forward pass!'
            feat1, feat2, pos1, pos2, cnn_feats_fm1, cnn_feats_fm2 = self._encode_image_pairs_onlyfm(img1[::2], img2[::2], shape1[::2], shape2[::2])
            feat1, feat2 = interleave(feat1, feat2)
            pos1, pos2 = interleave(pos1, pos2)
            cnn_feats_fm1, cnn_feats_fm2 = interleave_list(cnn_feats_fm1, cnn_feats_fm2)
        else:
            feat1, feat2, pos1, pos2, cnn_feats_fm1, cnn_feats_fm2  = self._encode_image_pairs_onlyfm(img1, img2, shape1, shape2)

        return (shape1, shape2), (feat1, feat2), (pos1, pos2), (cnn_feats_fm1, cnn_feats_fm2)
    ...
    def forward(self, view1, view2):
        ...
        (shape1, shape2), (feat1, feat2), (pos1, pos2), (cnn_feats_warp1, cnn_feats_warp2),  (cnn_feats_fm1, cnn_feats_fm2) = self._encode_symmetrized(view1, view2)
        ...
        cnn_feats_warp1.append(feat16_1)
        cnn_feats_warp2.append(feat16_2)

        cnn_feats_fm1.append(feat16_1)
        cnn_feats_fm2.append(feat16_2)
        import warnings
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=FutureWarning)
            with torch.cuda.amp.autocast(enabled=False):
                corresps = self._downstream_headwarp('warp', cnn_feats_warp1, cnn_feats_warp2, shape1, shape2)
                res1 = self._downstream_head(1, cnn_feats_fm1, shape1, upsample = False, low_desc = None, low_certainty = None)
                res2 = self._downstream_head(2, cnn_feats_fm2, shape2, upsample = False, low_desc = None, low_certainty = None)
        
        return corresps, res1, res2
    def onlyfm_forward(self, view1, view2):
        # encode the two images --> B,S,D
        (shape1, shape2), (feat1, feat2), (pos1, pos2), (cnn_feats1, cnn_feats2) = self._encode_symmetrized_onlyfm(view1, view2)
