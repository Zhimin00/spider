...
def train(args):
    ...
    if args.pretrained and not args.resume:
        ...
        keys_to_delete = [k for k in state_dict.keys() if k.startswith("downstream_headwarp")]
    ...
    print('trainable head parameters:', sum([p.numel() for n,p in model.downstream_headwarp.named_parameters() if p.requires_grad]))
    if args.distributed:
    ...
    # cnn_params = set(p for p in model_without_ddp.cnn.parameters())
    # head_params = set(p for p in model_without_ddp.downstream_head.parameters())
    # other_params = [p for p in model_without_ddp.parameters()
    #                 if p not in cnn_params and p not in head_params]
    ...
    # param_groups = [
        ...
        # {"params": model_without_ddp.downstream_head.parameters(), "lr": 32 * 1e-5 / 8},
    # ]
    param_groups = [
