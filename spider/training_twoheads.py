...
def get_args_parser():
    ...
    parser.add_argument('--pretrained_fm', default=None, help='path of a starting checkpoint')
...
def train(args):
    ...
    if args.test_criterion1 is not None:
        test_criterion1 = eval(args.test_criterion1).to(device)
    else:
        test_criterion1 = None
    if args.test_criterion2 is not None:
        test_criterion2 = eval(args.test_criterion2).to(device)
    else:
        test_criterion2 = None
    ...
    if args.pretrained and not args.resume:
        ...
        print('Loading pretrained fm:', args.pretrained_fm)
        fm_ckpt = torch.load(args.pretrained_fm, map_location=device)['model']
        ...
        del ckpt, warp_ckpt, fm_ckpt#, renamed  # in case it occupies memory
