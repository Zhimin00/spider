...
### rewrite dataset to load coarse image and fine image
### spider_match
...
if __name__ == '__main__':
    ...
    model = SPIDER.from_pretrained(weights_path).to(args.device)
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        for map_view in map_views:
            ...
            if cache_file is not None and os.path.isfile(cache_file):
                ...
                # coarse matching
                spider_match_path(model, device, im_A_path, im_B_path, coarse_size, fine_size)
